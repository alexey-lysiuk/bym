...
if sys.hexversion < 0x3090000:
    raise Exception('This module requires Python 3.7 or newer')
...
#
#
# def _add_prerequisites(config: 'Configuration', targets: typing.Iterable[str]) -> None:
#     prerequisites = OrderedSet(config.prerequisites)
#
...
def build(arguments):
    ...
    #
    # _add_prerequisites(config, targets)
    #
    # # Prepare build directories
    # _make_directory(config.state_path)
    # _make_directory(config.cache_path)
    # _make_directory(config.build_path)
    # _make_directory(config.bin_path)
    # _make_directory(config.include_path)
    # _make_directory(config.lib_path)
    #
    # # Do build
    # os.chdir(config.build_path)
    #
    # for target in targets:
    #     repository.package(target).build(config)
